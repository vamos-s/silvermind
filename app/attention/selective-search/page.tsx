    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-purple-50 p-4 md:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        {gameState === 'menu' && (
          ...
          >
            ...
            <div className="bg-violet-50 rounded-xl p-4 mb-6 text-left">
              <h3 className="font-bold text-gray-800 mb-2">{t('selectiveSearch.levelInfo', 'Level Settings')}:</h3>
          ...
          <>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
              <div className="flex justify-around text-center">
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('selectiveSearch.level', 'Level')}</p>
                  <p className="text-3xl font-bold text-violet-600">{level}/{MAX_LEVELS}</p>
                </div>
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('selectiveSearch.score', 'Score')}</p>
                  <p className="text-3xl font-bold text-purple-600">{totalScore}</p>
                </div>
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('selectiveSearch.timeLeft', 'Time')}</p>
                  <p className={`text-3xl font-bold ${timeLeft <= 5 ? 'text-red-500' : 'text-violet-600'}`}>{timeLeft}s</p>
                </div>
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('selectiveSearch.progress', 'Progress')}</p>
                  <p className="text-3xl font-bold text-violet-600">{foundTargets}/{totalTargets}</p>
                </div>
              </div>
              {/* Progress Bar */}
            ...
            <div className="bg-white rounded-2xl shadow-lg p-4 mb-6 text-center">
              <p className="text-gray-700 font-medium mb-2">{t('selectiveSearch.findThisShape', 'Find this shape:')}</p>
              <div className="text-5xl">{targetShape}</div>
              <p className="text-gray-700 font-medium mt-2">{t('selectiveSearch.ignoringColors', '(ignore the colors!)')}</p>
            </div>
          ...
          >
            ...
            <div className="bg-violet-50 rounded-xl p-6 mb-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('selectiveSearch.levelScore', 'Level Score')}</p>
                  <p className="text-3xl font-bold text-violet-600">{Math.round(score)}</p>
                </div>
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('selectiveSearch.totalScore', 'Total Score')}</p>
        ...
        {gameState === 'gameOver' && (
          ...
          >
            <div className="text-6xl mb-4">❌</div>
            <h2 className="text-4xl font-bold text-gray-800 mb-4">{t('selectiveSearch.gameOver', 'Time\'s Up!')}</h2>
            <div className="bg-orange-50 rounded-xl p-6 mb-6">
              <p className="text-gray-700 text-sm font-medium">{t('selectiveSearch.finalScore', 'Final Score')}</p>
        ...
        {gameState === 'victory' && (
          ...
          >
            <div className="text-6xl mb-4">🎉</div>
            <h2 className="text-4xl font-bold text-violet-600 mb-4">{t('selectiveSearch.victory', 'Congratulations!')}</h2>
