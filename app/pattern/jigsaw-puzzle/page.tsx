    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-pink-50 p-4 md:p-8">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {gameState === 'menu' && (
          ...
          >
            ...
            <div className="bg-rose-50 rounded-xl p-4 mb-6 text-left">
              <h3 className="font-bold text-gray-800 mb-2">{t('jigsawPuzzle.levelInfo', 'Level Settings')}:</h3>
            ...
            <button
              onClick={startLevel}
              className="bg-gradient-to-r from-rose-500 to-pink-500 text-white text-2xl font-bold py-4 px-12 rounded-xl hover:from-rose-600 hover:to-pink-600 shadow-lg transition-all w-full"
            >
              {t('start', 'Start')}
            </button>
          </motion.div>
          ...
          <>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('jigsawPuzzle.level', 'Level')}</p>
                  <p className="text-3xl font-bold text-rose-600">{level}/{MAX_LEVELS}</p>
                </div>
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('jigsawPuzzle.moves', 'Moves')}</p>
                  <p className="text-3xl font-bold text-gray-600">{moves}</p>
                </div>
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('jigsawPuzzle.timeLeft', 'Time Left')}</p>
                    ...
                    <motion.button
                      ...
                      className={`aspect-square rounded-lg font-bold text-2xl flex items-center justify-center transition-all ${
                        isSelected
                          ...
                          : 'hover:scale-105'
                      } ${piece ? COLORS[piece.correctPosition % COLORS.length] : 'bg-gray-200'}`}
                    >
          ...
          >
            ...
            <div className="bg-rose-50 rounded-xl p-6 mb-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('jigsawPuzzle.moves', 'Moves')}</p>
                  <p className="text-3xl font-bold text-rose-600">{moves}</p>
                </div>
                <div>
                  <p className="text-gray-700 text-sm font-medium">{t('jigsawPuzzle.levelScore', 'Level Score')}</p>
                  <p className="text-3xl font-bold text-rose-600">{score}</p>
                </div>
              </div>
              <div className="mt-4">
                <p className="text-gray-700 text-sm font-medium">{t('jigsawPuzzle.totalScore', 'Total Score')}</p>
                <p className="text-4xl font-bold text-pink-600">{totalScore + score}</p>
              </div>
            </div>
            <button
              onClick={nextLevel}
              className="bg-gradient-to-r from-rose-500 to-pink-500 text-white text-2xl font-bold py-4 px-12 rounded-xl hover:from-rose-600 hover:to-pink-600 shadow-lg transition-all w-full mb-3"
            >
        ...
        {gameState === 'gameOver' && (
          ...
          >
            <div className="text-6xl mb-4">⏰</div>
            <h2 className="text-4xl font-bold text-gray-800 mb-4">{t('jigsawPuzzle.timeUp', 'Time is up!')}</h2>
        ...
        {gameState === 'victory' && (
          ...
          >
            <div className="text-6xl mb-4">🎉</div>
            <h2 className="text-4xl font-bold text-rose-600 mb-4">{t('jigsawPuzzle.victory', 'Congratulations!')}</h2>
            <p className="text-xl text-gray-700 font-medium mb-6">
              {t('jigsawPuzzle.victoryMessage', 'You completed all {count} levels!', { count: MAX_LEVELS })}
            </p>
            <div className="bg-rose-50 rounded-xl p-6 mb-6">
              <p className="text-gray-700 text-sm font-medium">{t('jigsawPuzzle.finalScore', 'Final Score')}</p>
